...
Set `FEISHU_BOT_LLM_BASE_URL`, `FEISHU_BOT_LLM_API_KEY`, and `FEISHU_BOT_LLM_MODEL` to enable the optional OpenAI-compatible query planner. When enabled, the model interprets natural-language queries, short follow-ups, and multiple questions in one message, then produces up to three validated tool plans. Supported filters include machine group, GPU count, minimum free GPU memory, and idle status. A read-only usage-ranking tool reports high-resource users from the current hourly aggregation and groups machine accounts by the registered display name. The model receives the user's current text, the previous query plan, and a compact inventory of group/type counts; live metrics, passwords, approval records, and SSH credentials are not sent to it. Live results are always calculated locally and rendered from the dashboard snapshot. Destructive or privileged operations remain protected by fixed backend allowlists and role checks.
